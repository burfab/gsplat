...
class Runner:
    ...
    def __init__(
        ...
        if not self.cfg.disable_viewer:
            ...
            self.viewer = GsplatViewer(
                ...
                rasterizer=self.cfg.rasterizer,
                mode="training",
            )
    ...
    def stereo_compute_depthmap(self, Ks, width, height, c2w, ALPHA_TH=0.75, MAX_DIST_REL_TO_BASELINE = 50):
        ...
        torch.cuda.synchronize()
        right_image , _, _, _, _, _, _ = self.rasterize_splats(
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            # forward
            renders, alphas, normals, normals_from_depth, render_distort, render_median,info = self.rasterize_splats(
                ...
                image_ids=image_ids,
                render_mode=("RGB+ED" if (stereo_depth is not None) or cfg.depth_loss else "RGB") if self.cfg.rasterizer=="3dgs" else "RGB+D",
                #masks=masks,
            )
            ...
            normal_lambda = 0
            if cfg.normal_loss:
                assert self.cfg.rasterizer == "2dgs"
            ...
            dist_loss = torch.tensor(0.0).to(l1loss.device).requires_grad_(True)
            dist_lambda = 0
            if cfg.dist_loss:
                assert self.cfg.rasterizer == "2dgs"
    ...
    def _viewer_render_fn(
        ...
        RENDER_MODE_MAP = {
            "rgb": "RGB",
            "depth(accumulated)": "RGB+D",
            "depth(expected)": "RGB+ED",
            "depth(median)": "RGB+ED",
            "dist": "RGB+ED",
            "normal": "RGB+ED",
            "alpha": "RGB",
